...
export default {
    data: {
        ...
        options: [
            ...
            { name: 'language', type: 3, description: 'Target language (choose from list)', required: true, choices: languageChoices }
        ]
    },
    async execute(interaction) {
        try {
            ...
            if (!interaction.replied) await interaction.reply('⚠️ Error translating text. Please try again.');
            else await interaction.editReply('⚠️ Error translating text. Please try again.');
        }
    }
};
